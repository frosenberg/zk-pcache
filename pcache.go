...
func NewPathCache(path string, depth int, conn *zk.Conn) (pc *PathCache, notifyChan chan PathCacheNotification, err error) {
	pc = &PathCache{
		path:     path,
		depth:    depth,
...
func (pc *PathCache) watch() error {
	...
	value, stat, ev, err := pc.conn.GetW(pc.path)
...
func (pc *PathCache) watchChildren() error {
	...
	children, _, ev, err := pc.conn.ChildrenW(pc.path)
...
func (pc *PathCache) mux() {
	for {
		...
		case p := <-pc.addChildChan:
			...
			pchild, nChan, err := NewPathCache(path, depth, pc.conn)
			if err != nil {
				log.Error("Error creating child path", err.Error())
				pc.startup.Done()
				continue
			}
